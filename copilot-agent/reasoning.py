...
def analyze_incident(bundle: EvidenceBundle) -> RootCauseHypothesis:
    ...
    hypothesis = parse_and_validate_hypothesis(response_text, bundle)
    logger.info(
        "LLM result: status=%s confidence=%s cited_ids=%d",
        hypothesis.result_status,
        hypothesis.confidence,
        len(hypothesis.cited_ids),
    )
...
def parse_and_validate_hypothesis(
    ...
    try:
        ...
        return _result("LLM_FAILED", "LLM response was not valid RootCauser JSON.")
    ...
    hypothesis.result_status = (
        "INSUFFICIENT_EVIDENCE" if hypothesis.insufficient_evidence else "SUCCESS"
    )
    if hypothesis.insufficient_evidence:
        hypothesis.confidence = "Insufficient Evidence"
    return hypothesis
...
def _call_llm(bundle: EvidenceBundle) -> str:
    ...
    if not settings.llm_api_key:
        return json.dumps(
            {
                "summary": (
                    "LLM_API_KEY is not configured, so "
                    "RootCauser cannot produce a grounded hypothesis."
                ),
                "cited_ids": [],
    ...
    payload = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": (f"{investigation_prompt}\n\n{reasoning_prompt}"),
            },
        ],
        "temperature": 0.1,
        "response_format": {
    ...
    for attempt in (1, 2):
        try:
            ...
            logger.warning("OpenRouter request failed (attempt %d/2): %s", attempt, exc)
            ...
            return json.dumps(
                {
                    "summary": (f"OpenRouter request failed after retry: {exc}"),
                    "cited_ids": [],
                    "suggested_fix": (
                        "Check OpenRouter credentials, network access, and model name."
                    ),
                    "insufficient_evidence": True,
                }
            )
...
def _insufficient(
    ...
    return _result("INSUFFICIENT_EVIDENCE", summary)


def _result(status: str, summary: str) -> RootCauseHypothesis:
