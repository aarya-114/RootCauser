...
class AlertManager:
    ...
    def ensure_alert(
        ...
        if existing_alert is None:
            print(
                f"[CREATE] Alert does not exist: {alert_name}"
            )
            return self.create_alert(payload)

        if not alerts_differ(existing_alert, payload):
            print(
                f"[NOOP] Alert already matches: {alert_name}"
            )
            return existing_alert

        print(
            f"[UPDATE] Alert configuration changed: {alert_name}"
        )
